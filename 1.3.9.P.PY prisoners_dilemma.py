...
def get_action(player, history, opponent_history, score, opponent_score, getting_team_name=False):
    ...
    #
        ...
        if getting_team_name:
            ...
            countB=0 #count variable to count number of 'b'
            countC=0 #count variable to count number of 'c'
            for char in opponent_history: #for loop to check each 
        #character in opponent history
                if char == "b":    #checking for 'b'
                    countB=countB+1 
                else:              #checking for 'c'
                    countC=countC+1
            ''' will return 'b' if opponent backstabs more than colludes, 
      will return 'c' if opponent colludes more or if number of backstabs 
      and colludes are equal'''             
